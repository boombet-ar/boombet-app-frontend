...
class EventoDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final effectiveValue = options.any((o) => o.id == selectedId)
        ? selectedId
        : null;
    return Container(
...
class StandOption {
  ...
  const StandOption({required this.id, required this.label});
}

const List<StandOption> kDefaultStandOptions = [
  StandOption(id: null, label: 'Sin stand'),
];

class StandDropdown extends StatelessWidget {
  final List<StandOption> options;
  final int? selectedId;
  final Color accent;
  final Color textColor;
  final Color bgColor;
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int?>(
          ...
          dropdownColor: bgColor,
          icon: Icon(Icons.keyboard_arrow_down, color: accent),
          hint: Text('Stand', style: TextStyle(color: textColor, fontSize: 14)),
          items: options
              .map(
                (opt) => DropdownMenuItem<int?>(
                  value: opt.id,
